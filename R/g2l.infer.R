g2l.infer <-
function(X,z,X.test=NULL,m=c(6,8),alpha=.1,nsample=length(z), fdr.curve.approx=c("direct", "indirect"),
                    ngrid=2000,centering='LP',fdr.method=c("locfdr","BH"), locfdr.df=10, coef.smooth='BIC',
					fdr.th.fixed=NULL,rel.null=c('custom','th'),parallel=FALSE){
  X<-as.matrix(X)
  n<-length(z)
  X.axe<-unique(X)
